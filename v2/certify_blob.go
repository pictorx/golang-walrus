...
func CompleteWalrusFlow2(
	...
	regResp, err := register.ReserveAndRegisterBlob2(conn, acc, ctx)
	if err != nil {
		return fmt.Errorf("register: %w", err)
	}
	...
	if config.SendTip != nil {
		...
		tipResp, nonceStr, err = PayRelayTip2(
			ctx, conn, acc,
			blobData,
			*config.SendTip.Kind.Const,
			config.SendTip.Address,
	...
	certifyResp, err := certTx.CertifyBlob2(conn, acc, ctx)
	if err != nil {
		return fmt.Errorf("certify: %w", err)
	}
