...
// BUG FIX: Was `#[cfg(feature = "rpc")]` but the feature is called "intents"
// in Cargo.toml.  The wrong flag caused the entire async impl block to be
// compiled out, so `CoinWithBalanceResolver::resolve` never ran and the intent
